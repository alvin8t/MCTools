...
function appendTemplate(node, templateName){
	...
	if (templateName == 'pool-elem') template.classList.add('tab-pane')
	if (templateName == 'pool-elem') template.id = 'p' + poolCount;
	node.appendChild(template);
	
	if (templateName == 'pool-elem') {
		var pillsAdd = document.getElementById("pill-temp").getElementsByClassName("tab");
		var pillAppend = pillsAdd[0].cloneNode(true);
		pillAppend.href = '#p' + poolCount;
		pillAppend.id = 'p' + poolCount;
		var poolLI = document.createElement('li');
		if (poolCount == 0) {
			poolLI.classList.add('active')
		}
		poolLI.appendChild(pillAppend)
		var poolAppend = document.getElementById("poolAppend");
		poolAppend.appendChild(poolLI)
